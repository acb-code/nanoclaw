...
When a user sends a PDF via a configured channel (Telegram, WhatsApp, etc.), the channel's attachment handler saves it to the group's workspace — `sources/` for wiki groups, `attachments/` otherwise. The incoming message includes a path hint like:
...
```bash
pdf-reader extract sources/paper.pdf --layout
```
...
`pdftotext` only reads PDFs with an embedded text layer. Scanned or image-only PDFs will return empty output. OCR is out of scope for this skill.
